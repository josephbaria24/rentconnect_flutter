...
class _AgreementDetailsState extends State<AgreementDetails> {
  ...
  Future<void> fetchAgreementDetails() async {
    final String apiUrl = 'http://192.168.1.18:3000/rental-agreement/inquiry/${widget.inquiryId}';
    ...
    if (agreement != null) {
      try {
        ...
        final landlordResponse = await http.get(
          Uri.parse('http://192.168.1.18:3000/user/$landlordId'), // Correct URL
        );
        ...
        final occupantResponse = await http.get(
          Uri.parse('http://192.168.1.18:3000/user/$occupantId'), // Correct URL
        );
