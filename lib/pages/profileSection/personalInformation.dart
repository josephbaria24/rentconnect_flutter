...
Future<void> _updateProfileCompletion() async {
  final url = Uri.parse('http://192.168.1.6:3000/profile/updateProfile');
  try {
  ...
  Future<void> _updateRole() async {
    final url = Uri.parse('http://192.168.1.6:3000/updateProfile');
    try {
      final response = await http.patch(
        url,
        headers: {
  ...
  Future<void> _uploadValidId() async {
    if (_validIdImage != null) {
      var request = http.MultipartRequest(
        'PATCH',
        Uri.parse('http://192.168.1.6:3000/profile/uploadValidId')
      );
